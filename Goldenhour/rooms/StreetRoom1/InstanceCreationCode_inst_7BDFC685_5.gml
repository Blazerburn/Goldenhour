/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 54620D23
/// @DnDArgument : "var" "global.RoomSpawnpoints"
/// @DnDArgument : "value" "1"
if(global.RoomSpawnpoints == 1){	/// @DnDAction : YoYo Games.Instances.Create_Instance
	/// @DnDVersion : 1
	/// @DnDHash : 09972AC6
	/// @DnDParent : 54620D23
	/// @DnDArgument : "xpos" "32"
	/// @DnDArgument : "ypos" "256"
	/// @DnDArgument : "objectid" "TestPlayer"
	/// @DnDArgument : "layer" ""Player""
	/// @DnDSaveInfo : "objectid" "TestPlayer"
	instance_create_layer(32, 256, "Player", TestPlayer);}

/// @DnDAction : YoYo Games.Common.Else
/// @DnDVersion : 1
/// @DnDHash : 5BCD2002
else{	/// @DnDAction : YoYo Games.Common.If_Variable
	/// @DnDVersion : 1
	/// @DnDHash : 5C63F3BF
	/// @DnDParent : 5BCD2002
	/// @DnDArgument : "var" "global.RoomSpawnpoints"
	/// @DnDArgument : "value" "2"
	if(global.RoomSpawnpoints == 2){	/// @DnDAction : YoYo Games.Instances.Create_Instance
		/// @DnDVersion : 1
		/// @DnDHash : 4843F3DE
		/// @DnDParent : 5C63F3BF
		/// @DnDArgument : "xpos" "288"
		/// @DnDArgument : "ypos" "352"
		/// @DnDArgument : "objectid" "TestPlayer"
		/// @DnDArgument : "layer" ""Player""
		/// @DnDSaveInfo : "objectid" "TestPlayer"
		instance_create_layer(288, 352, "Player", TestPlayer);}

	/// @DnDAction : YoYo Games.Common.Else
	/// @DnDVersion : 1
	/// @DnDHash : 3BA1C846
	/// @DnDParent : 5BCD2002
	else{	/// @DnDAction : YoYo Games.Common.If_Variable
		/// @DnDVersion : 1
		/// @DnDHash : 0C4A7A36
		/// @DnDParent : 3BA1C846
		/// @DnDArgument : "var" "global.RoomSpawnpoints"
		/// @DnDArgument : "value" "3"
		if(global.RoomSpawnpoints == 3){	/// @DnDAction : YoYo Games.Instances.Create_Instance
			/// @DnDVersion : 1
			/// @DnDHash : 32B53A70
			/// @DnDParent : 0C4A7A36
			/// @DnDArgument : "xpos" "960"
			/// @DnDArgument : "ypos" "256"
			/// @DnDArgument : "objectid" "TestPlayer"
			/// @DnDArgument : "layer" ""Player""
			/// @DnDSaveInfo : "objectid" "TestPlayer"
			instance_create_layer(960, 256, "Player", TestPlayer);}
	
		/// @DnDAction : YoYo Games.Common.Else
		/// @DnDVersion : 1
		/// @DnDHash : 0E666CBA
		/// @DnDParent : 3BA1C846
		else{	/// @DnDAction : YoYo Games.Common.If_Variable
			/// @DnDVersion : 1
			/// @DnDHash : 0E56A4D0
			/// @DnDParent : 0E666CBA
			/// @DnDArgument : "var" "global.RoomSpawnpoints"
			/// @DnDArgument : "value" "4"
			if(global.RoomSpawnpoints == 4){	/// @DnDAction : YoYo Games.Instances.Create_Instance
				/// @DnDVersion : 1
				/// @DnDHash : 21D7D690
				/// @DnDParent : 0E56A4D0
				/// @DnDArgument : "xpos" "480"
				/// @DnDArgument : "ypos" "32"
				/// @DnDArgument : "objectid" "TestPlayer"
				/// @DnDArgument : "layer" ""Player""
				/// @DnDSaveInfo : "objectid" "TestPlayer"
				instance_create_layer(480, 32, "Player", TestPlayer);}
		
			/// @DnDAction : YoYo Games.Common.Else
			/// @DnDVersion : 1
			/// @DnDHash : 4D832EAB
			/// @DnDParent : 0E666CBA
			else{	/// @DnDAction : YoYo Games.Common.If_Variable
				/// @DnDVersion : 1
				/// @DnDHash : 179C7D65
				/// @DnDParent : 4D832EAB
				/// @DnDArgument : "var" "global.RoomSpawnpoints"
				/// @DnDArgument : "value" "5"
				if(global.RoomSpawnpoints == 5){	/// @DnDAction : YoYo Games.Instances.Create_Instance
					/// @DnDVersion : 1
					/// @DnDHash : 590BCC58
					/// @DnDParent : 179C7D65
					/// @DnDArgument : "xpos" "704"
					/// @DnDArgument : "ypos" "352"
					/// @DnDArgument : "objectid" "TestPlayer"
					/// @DnDArgument : "layer" ""Player""
					/// @DnDSaveInfo : "objectid" "TestPlayer"
					instance_create_layer(704, 352, "Player", TestPlayer);}
			
				/// @DnDAction : YoYo Games.Common.Else
				/// @DnDVersion : 1
				/// @DnDHash : 49D6CF25
				/// @DnDParent : 4D832EAB
				else{	/// @DnDAction : YoYo Games.Common.If_Variable
					/// @DnDVersion : 1
					/// @DnDHash : 124830AC
					/// @DnDParent : 49D6CF25
					/// @DnDArgument : "var" "global.RoomSpawnpoints"
					/// @DnDArgument : "value" "6"
					if(global.RoomSpawnpoints == 6){	/// @DnDAction : YoYo Games.Instances.Create_Instance
						/// @DnDVersion : 1
						/// @DnDHash : 12EFE537
						/// @DnDParent : 124830AC
						/// @DnDArgument : "xpos" "176"
						/// @DnDArgument : "ypos" "176"
						/// @DnDArgument : "objectid" "TestPlayer"
						/// @DnDArgument : "layer" ""Player""
						/// @DnDSaveInfo : "objectid" "TestPlayer"
						instance_create_layer(176, 176, "Player", TestPlayer);}}}}}}