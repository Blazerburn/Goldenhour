			...
			if(global.pushingBox == 1)
	{
		if (global.Talking == 0) {
			...
			var pushableBoxOriginX = x;
				...
				if _dont = false {
					if (pushPlayerDirection == 0 || pushPlayerDirection == 2) {
					move_and_collide(xinput * SlowSpeed, yinput * NormalSpeed, [obj_PushableObject, obj_StuckPuzzleBox, PushableWall, pushCollision],4,0,0,SlowSpeed,NormalSpeed);
					}
					if (pushPlayerDirection == 1 || pushPlayerDirection == 3) {
					move_and_collide(xinput * NormalSpeed, yinput * SlowSpeed, [obj_PushableObject, obj_StuckPuzzleBox, PushableWall, pushCollision],4,0,0,NormalSpeed,SlowSpeed);
					}
				}
